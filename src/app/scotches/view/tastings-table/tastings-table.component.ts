...
@Component({
  ...
  templateUrl: './tastings-table.template.html',
...
export class TastingsTableComponent implements OnInit {
  @Input() personalTastings: any[];
  @Input() scotch: any;
    ...
    this._confirmDialogService.dialogResponded.subscribe(
      response => {
        if (response.confirmed) {
          ...
          this.refreshNeeded.emit('tastings');
          });
        }
      });
